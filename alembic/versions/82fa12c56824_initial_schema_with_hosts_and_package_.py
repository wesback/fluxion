...
def upgrade() -> None:
    ...
    op.create_index('ix_package_updates_package_name', 'package_updates', ['package_name'], unique=False)
    op.create_index('ix_package_updates_update_timestamp', 'package_updates', ['update_timestamp'], unique=False)
    
    # Create composite indexes for efficient queries
    op.create_index('ix_package_updates_host_id_update_timestamp', 'package_updates', ['host_id', 'update_timestamp'], unique=False)
    op.create_index('ix_package_updates_package_name_update_timestamp', 'package_updates', ['package_name', 'update_timestamp'], unique=False)


def downgrade() -> None:
    """Downgrade schema."""
    # Drop package_updates table and its indexes
    op.drop_index('ix_package_updates_package_name_update_timestamp', table_name='package_updates')
