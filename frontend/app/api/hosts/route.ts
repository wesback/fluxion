...
/**
 * API route for fetching hosts from the backend
...
export async function GET() {
  ...
  try {
    const response = await axios.get(`${API_BASE_URL}/api/v1/hosts`, {
      headers: {
        'X-API-Key': API_KEY,
      },
      timeout: 10000,
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error(`Error fetching hosts: ${status} - ${message}`);
      return NextResponse.json(
        { error: message },
        { status }
      );
